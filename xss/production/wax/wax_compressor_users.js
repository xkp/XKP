    ...
    SessionManager.httpRequest(request, response, function (err, session)
    {
        ...
        }).on('end', function ()
        {
            ...
            __connection1.connect(function (error)
            {
                ...
                {
                    if (error) throw error;
                    if (rows.length == 0) user_data = null;
                    else if (rows.length == 1) user_data = rows[0];
                    else user_data = rows;
                    __connection1.end();
                    __callback1();
                });
...
application.do_register = function (request, response)
{
    ...
    }).on('end', function ()
    {
        ...
        function __callback2()
        {
            ...
            var __ws_url1 = "http://maps.googleapis.com/maps/api/staticmap" + "?center=" + map_center + "&zoom=" + zoom + "&sensor=" + false + "&size=" + "300x200";
...
application.file_list = function (request, response)
{
    SessionManager.httpRequest(request, response, function (err, session)
    {
        ...
        function return_function()
        {
            ...
            response.write("<b id=\"user_name\">" + user_name.inner_html + "</b>");
            ...
            for (var files_iterator = 0; files_iterator < files.data.length; files_iterator++)
            {
                ...
                link.href = item.Url;
                response.write("<li>\n                ");
                response.write("<b id=\"file_name\">" + file_name.inner_html + "</b>");
                response.write(": ");
    ...
    SessionManager.httpRequest(request, response, function (err, session)
    {
        ...
        }).on('end', function ()
        {
            ...
            function return_function()
            {
                ...
                response.write("<b id=\"file\">" + file.inner_html + "</b>");
                response.write("<br>\n            Here is your ");
                response.write("<a href = ");
                response.write('\"' + link.href + '\"');
                response.write("id = \"link\">");
...
http.createServer(function (request, response)
{
    ...
    else if (pathname == '/do_login')
    {
        application.do_login(request, response);
    }
    else if (pathname == '/logout')
    {
        application.logout(request, response);
    }
    else if (pathname == '/register')
    {
        application.register(request, response);
    }
    else if (pathname == '/do_register')
    {
        application.do_register(request, response);
    }
    else if (pathname == '/file_list')
    {
        application.file_list(request, response);
    }
