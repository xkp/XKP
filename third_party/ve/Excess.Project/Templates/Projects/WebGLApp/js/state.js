...
state.State = Class.extend(
{
    init: function(start, end, handler)
    {
		this.start 	 = start;
		this.end   	 = end;
...
state.Unit = Class.extend(
{
    ...
    update: function(t, pt)
    {
		...
		if ((flags & state.BOOLEAN) != 0)
		{
			this.update_boolean(t);
			return;
		}

		if ((flags & state.PUNCTUAL) != 0)
		{
    	    for(var i = 0; i < this.states.length; i++)
    	    {
    		    var st = this.states[i];
    	...
    	{
    		var st 	  = this.states[i];
    		if ((flags & state.RELATIVE) == 0)
    			from = i;

    		use_nearest = (flags & state.USE_NEAREST) != 0;
    		if (st.start > t)
    		{
        		if (use_nearest)
        		{
    ...
    build: function(start, end)
    {
    	...
    	if (!end)
    	{
    		...
    		for(var i = 0; i < this.states.length; i++)
    		{
    			var state_ = this.states[i];
    			if (end < state_.end) end = state_.end;
    		}
    	...
    	for(var i = 0; i < this.actions.length; i++)
    	{
    		...
    		new_states.push(new state.State(act.start, act.end, act.handler));
    ...
    resolve: function(st1, st2)
    {
    	...
    	if (st1.start < shared_start)
    	{
    		...
    		var only_st1 = new state.State(st1.start, shared_start, handler);
    	...
    	else if (st2.start < shared_start)
    	{
    		...
    		var only_st2 = new state.State(st2.start, shared_start, handler);
		...
		var shared = new state.State(shared_start, shared_end);
    	...
    	if (st1.end > shared_end)
    	{
    		...
    		var only_st1 = new state.State(shared_end, st1.end, handler);
    		result.push(only_st1);
    	}
    	else if (st2.end > shared_end)
    	{
    		var handler  = st2.handler.split((shared_end - st2.start)/(st2.end - st2.start), 1);
    		var only_st2 = new state.State(shared_end, st2.end, st2.handler);
    		result.push(only_st2);
    	}
...
state.Sequence = Class.extend(
{
	...
	get_handler: function(owner, channel, time)
	{
		...
		var state_ = channel_unit.get_state(time);
		if (!state_)
			 return null;
	...
	dump_states: function()
	{
		for(var i = 0; i < this.units.length; i++)
		{
			...
			{
				var state_ = unit.states[j];
				console.log("start:   " + state_.start);
				console.log("end:     " + state_.end);
				console.log("handler: " + state_.handler.get_class());

				if ("dump" in state_.handler)
					state_.handler.dump();
			}
		}
	}
...
//Handlers
state.Interpolator = state.Handler.extend(
{
    init: function(channel, interp_fn, assign_fn, keys)
    {
...
state.Caller = state.Handler.extend(
{
    init: function(fn)
    {
        this.fn = fn;
...
state.Every = state.Handler.extend(
{
