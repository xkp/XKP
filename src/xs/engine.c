...
#include <xs/engine.h>              /* Defines all the data structs. */
        ...
        if (*InputHere < InputSize) 
          {
            if (InputBuf[*InputHere] == '\r') 
              {
                if ((*InputHere + 1 < InputSize) && (InputBuf[*InputHere + 1] != '\n')) 
                  {
                    *Line = *Line + 1;
                    *Column = 0;
                  }
...
int FindChar(wchar_t ThisChar, wchar_t *CharacterSet, long Count) 
  {
    ...
    if (Count < 11) 
      {
        if (wcschr(CharacterSet, ThisChar) != NULL) 
          return (1);
        ...
        for (i = 0; i < Grammar.DfaArray[DfaIndex].EdgeCount; i++) 
          {
            if (FindChar(InputBuf[*InputHere + Length],
                  ...
                  Grammar.DfaArray[DfaIndex].Edges[i].CharCount) == 1) 
              break;
          }

        /* If not found then exit the loop. */
        if (i >= Grammar.DfaArray[DfaIndex].EdgeCount) 
          break;
...
void DeleteStack(struct TokenStackStruct *Stack) 
  {
    ...
    while (Stack != NULL) 
      {
        DeleteTokens(Stack->Token);
        OldTop = Stack;
        Stack = Stack->NextToken;
        free(OldTop);
      }
  }
...
int Parse(
    ...
    struct TokenStackStruct **TopTokenStack)    /* Exported token stack when Parse return PARSESYNTAXERROR */
  {
    ...
    while (1) 
      {
        ...
        if (Grammar.SymbolArray[Work->Token->Symbol].Kind == SYMBOLCOMMENTSTART) 
          {
            if (Debug > 0) 
              fprintf(stdout,"Parse: skipping comment.\n");
